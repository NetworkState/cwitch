...
constexpr TOKEN MKV_MASTER_ELEMENTS[] = { MKV_EBML, MKV_Segment, MKV_SeekHead, MKV_Seek, MKV_Info, MKV_Cluster, MKV_Tracks,
...
constexpr bool IsMasterElement(TOKEN id)
{
...
struct MKV_MASTER_ELEMENT
{
	...
	MKV_MASTER_ELEMENT(TOKEN name, FILE_READER& reader, UINT64 size) : elementName(name), fileReader(reader), bytesLeft(size) {}
	...
	MKV_MASTER_ELEMENT readMasterElement(TOKEN childElement, UINT64 size)
	{
		this->bytesLeft -= size;
